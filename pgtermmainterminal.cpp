...
#include "./ui_pgtermmainterminal.h"
#include <QStatusBar>
#include <QVBoxLayout>
#include <QLabel>
#include <QTextBlock>
...
PgTermMainTerminal::PgTermMainTerminal(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::PgTermMainTerminal)
    , m_worker(new Worker)
...
//    connect(exitAction, &QAction::triggered, this, &PgTermMainTerminal::close);

    connect(ui->action_Exit_2, &QAction::triggered, this, &PgTermMainTerminal::close);

    connect(ui->action_Limpar, &QAction::triggered, this, &PgTermMainTerminal::limparTexto);
...
PgTermMainTerminal::~PgTermMainTerminal()
{
    m_thread->quit();
...
void PgTermMainTerminal::keyPressEvent(QKeyEvent *event) {
...
void PgTermMainTerminal::limparTexto() {
    ui->plainTextEdit->clear();  // Limpa todo o conteúdo do QPlainTextEdit
}
//*******************************************************************************************
//*******************************ON    SERIAL************************************************
//*******************************************************************************************
void PgTermMainTerminal::onSerialDataReceived(const QByteArray &data)
{
    appendTerminalText(QString::fromUtf8(data), Qt::cyan);
}

void PgTermMainTerminal::onSerialError(const QString &error)
{
    appendTerminalText("[ERRO SERIAL] " + error, Qt::red);
    statusBar()->showMessage(error, 5000);
}

void PgTermMainTerminal::onSerialStatusChanged(const QString &status)
{
...
//*************************************SERIAL************************************************
//*******************************************************************************************
void PgTermMainTerminal::setupSerial()
{
    connect(m_serial, &PdTermSerial::dataReceived,this, &PgTermMainTerminal::onSerialDataReceived);
    connect(m_serial, &PdTermSerial::errorOccurred,this, &PgTermMainTerminal::onSerialError);
    connect(m_serial, &PdTermSerial::statusChanged,this, &PgTermMainTerminal::onSerialStatusChanged);

    // Configuração inicial padrão
    m_serial->setBaudRate(115200);  // Baud rate padrão
}

void PgTermMainTerminal::setup_ui(){

    //Cursor
    ui->plainTextEdit->setCursorWidth(2);          // Largura do cursor

}
void PgTermMainTerminal::setup_connect(){
    // Conexões existentes...
    ...
    connect(ui->actionConfigura_o, &QAction::triggered,
            this, &PgTermMainTerminal::on_actionSerialSettings_triggered);
}

void PgTermMainTerminal::on_actionSerialSettings_triggered()
{
    QDialog dialog(this);
...
void PgTermMainTerminal::appendTerminalText(const QString &text, const QColor &color, bool newLine) {
...
void PgTermMainTerminal::setTextAtPosition(int row, int col, const QString &text, const QColor &color) {
...
void PgTermMainTerminal::ensureLineExists(int row) {
